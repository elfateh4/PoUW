...
def debug_temporal_detection():
    ...
    print(f"Intervals should be: {[burst_times[i] - burst_times[i-1] for i in range(1, len(burst_times))]}")

    for i, msg_time in enumerate(burst_times):
        result = detector.update_network_metrics(node_id, int(msg_time))
    ...
    result = detector.detect_temporal_anomalies(node_id, int(burst_times[-1]))
