...
class PoUWDemo:
    ...
    def setup_demo_network(self):
        ...
        for i in range(3):
            ...
            self.miners[miner_id] = PoUWMiner(miner_id, omega_b=1e-2, omega_m=1e-2)  # Higher coefficients for demo
        ...
        for verifier_id in self.verifiers:
            success = self.economic_system.buy_ticket(
                verifier_id, NodeRole.VERIFIER, 50.0, {}
            )
            if success:
    ...
    def simulate_ml_training_and_mining(self, iterations: int = 5):
        ...
        for iteration in range(iterations):
            ...
            for miner_id, trainer in trainers.items():
                ...
                self.logger.info(f"{miner_id} - Loss: {metrics['loss']:.4f}, Accuracy: {metrics['accuracy']:.4f}")
                
                # Attempt mining
                miner = self.miners[miner_id]
                
                # Set easy difficulty for demo
                self.blockchain.difficulty_target = 0x0FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF
                ...
                result = miner.mine_block(
                    ...
                    list(self.blockchain.mempool), self.blockchain
                )
                ...
                if result:
                    ...
                    for verifier_id, verifier in self.verifiers.items():
                        ...
                        self.logger.info(f"{verifier_id} verification: {'VALID' if is_valid else 'INVALID'}")
    ...
    def complete_task_and_evaluate(self):
        ...
        final_models = {'best_model': 'demo_model_hash'}  # In practice, this would be the actual best model
    ...
    def print_demo_results(self):
        ...
        total_stake = sum(ticket.stake_amount for ticket in self.economic_system.stake_pool.tickets.values())
