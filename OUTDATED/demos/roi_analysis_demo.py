...
def demonstrate_roi_analysis():
    ...
    miner_performance = {"performance_score": 0.92, "uptime": 0.98, "accuracy": 0.94}
    ...
    miner_roi = analyzer.calculate_participant_roi(
        ParticipantRole.MINER, miner_performance, market_conditions
    )
    ...
    supervisor_roi = analyzer.calculate_participant_roi(
        ParticipantRole.SUPERVISOR, supervisor_performance, market_conditions
    )
    ...
    # Medium-scale task
    medium_task = {"estimated_hours": 8, "gpu_count": 4, "complexity_multiplier": 1.2}
    ...
    print(
        f"  Initial Network Value: ${simulation['simulation_results'][0]['network_economics'].total_network_value:,.0f}"
    )
    ...
    for day in milestones:
        if day <= len(simulation["simulation_results"]):
            ...
            participants = (
                sum(result["participants"].values())
                - result["participants"][ParticipantRole.CLIENT]
            )
    ...
    for role, description in roles_to_analyze:
        performance = {
            "performance_score": 0.85 + (hash(role.value) % 10) / 100
        }  # Varied performance
    ...
    hardware_analysis = analyze_miner_profitability(
        hardware_specs, hardware_market_conditions, 0.91
    )

    print(f"Hardware-Specific Miner Analysis:")
    print(
        f"  Investment Recommendation: {hardware_analysis['profitability_assessment']['recommendation']}"
    )
    ...
    for scenario_name, scenario_conditions in scenarios:
        scenario_roi = analyzer.calculate_participant_roi(
            ParticipantRole.MINER, {"performance_score": 0.85}, scenario_conditions
        )
    ...
    for i in range(100):
        ...
        analyzer.calculate_participant_roi(ParticipantRole.MINER, test_performance, test_conditions)
    ...
    print(
        f"• Clients save {large_savings['savings_percentage']:.0f}% on large ML tasks vs cloud services"
    )
