...
class NetworkOperationsDemo:
    ...
    async def setup_network(self):
        ...
        for i in range(3):
            supervisor = PoUWNode(
                f"supervisor_{i:03d}", NodeRole.SUPERVISOR, "localhost", base_port + i
            )
        ...
        for i in range(5):
            worker = PoUWNode(f"worker_{i:03d}", NodeRole.MINER, "localhost", base_port + 10 + i)
    ...
    async def demonstrate_crash_recovery(self):
        ...
        new_worker = PoUWNode(
            crashed_worker.node_id, NodeRole.MINER, crashed_worker.host, crashed_worker.port
        )
    ...
    async def demonstrate_worker_replacement(self):
        ...
        for i in range(3):
            ...
            try:
                ...
                self.logger.info(
                    f"📝 Assigned {primary_workers[0]} to {task_id} (backup: {backup_workers[0] if backup_workers else 'none'})"
                )
    ...
    async def demonstrate_message_compression(self):
        ...
        if stats_after["average_compression_ratio"] > 0:
            self.logger.info(
                f"   Compression ratio: {stats_after['average_compression_ratio']:.2f}"
            )
