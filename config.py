...
class ConfigManager:
    ...
    def _load_from_env(self):
        ...
        self.config.deployment.vps_ssh_key_path = os.getenv('POUW_VPS_SSH_KEY_PATH', '~/.ssh/id_rsa')
        ...
        self.config.security.enable_attack_mitigation = self._get_bool('POUW_ENABLE_ATTACK_MITIGATION', True)
        self.config.security.enable_advanced_features = self._get_bool('POUW_ENABLE_ADVANCED_FEATURES', False)
        self.config.security.enable_production_features = self._get_bool('POUW_ENABLE_PRODUCTION_FEATURES', False)
        ...
        self.config.monitoring.dashboard_url = os.getenv('POUW_DASHBOARD_URL', 'http://localhost:8080')
        ...
        self.config.monitoring.health_check_interval = self._get_int('POUW_HEALTH_CHECK_INTERVAL', 30)
    ...
    def _validate_configuration(self):
        ...
        if self.config.environment == 'production':
            ...
            for field_name, field_value in required_fields:
                if not field_value or field_value in ['your-domain.com', 'admin@localhost', 'dev_secret_key']:
...
def get_config_manager(env_file: Optional[str] = None, environment: Optional[str] = None) -> ConfigManager:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--generate-keys", "-g", action="store_true", help="Generate new security keys")
