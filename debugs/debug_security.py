...
def test_gradient_poisoning_detection():
    ...
    print(f"  ✅ Krum: {len(clean_krum)}/{len(all_updates)} updates clean, {len(alerts_krum)} alerts")
    
    # Test Kardam filter (robust statistics)
    clean_kardam, alerts_kardam = detector.kardam_filter(all_updates)
    print(f"  ✅ Kardam: {len(clean_kardam)}/{len(all_updates)} updates clean, {len(alerts_kardam)} alerts")
    
    # Test combined detection
    clean_combined, alerts_combined = detector.detect_gradient_poisoning(all_updates, method="both")
    print(f"  ✅ Combined: {len(clean_combined)}/{len(all_updates)} updates clean, {len(alerts_combined)} alerts")
...
def test_byzantine_fault_tolerance():
    ...
    proposal_history = {proposal_id: {supervisor_id: {"vote": vote} for supervisor_id, vote in votes}}
