...
class AdvancedPoUWDemo:
    ...
    async def initialize_supervisor_dkg(self):
        ...
        try:
            ...
            if dkg_successful >= 2:  # Majority completed
                ...
                logger.info(f"✅ DKG completed successfully on {dkg_successful}/{len(self.supervisor_nodes)} supervisors")
            else:
    ...
    async def stake_and_register_nodes(self):
        ...
        staking_config = {
            ...
            NodeRole.MINER: {'amount': 150.0, 'preferences': {'has_gpu': True, 'model_types': ['mlp']}},
    ...
    async def submit_ml_task_with_advanced_selection(self):
        ...
        try:
            ...
            task_obj = client_node.current_task if client_node.current_task else MLTask(
                task_id=task_id,
    ...
    async def demonstrate_gradient_poisoning_detection(self):
        ...
        try:
            ...
            if alerts_count > 0:
                ...
                logger.info(f"  Latest alert: {latest_alert.alert_type.value} from {latest_alert.node_id}")
    ...
    async def demonstrate_zero_nonce_commitments(self):
        ...
        try:
            ...
            for i in range(3):
                ...
                logger.info(f"📋 Created commitment {commitment['commitment_id'][:16]}... for iteration {commitment['future_iteration']}")
    ...
    async def demonstrate_data_management(self):
        ...
        try:
            ...
            sample_data = [{'features': np.random.randn(10).tolist(), 'label': i % 3} for i in range(100)]
    ...
    async def run_demo(self):
        ...
        try:
            ...
            logger.info(f"  Gradient filtering events: {self.demo_results['gradient_filtering_events']}")
            
            # Node status summary
            logger.info("🏷️ Node Status Summary:")
            for node_id, node in self.nodes.items():
                status = node.get_status()
                logger.info(f"  {node_id}: {status['role']} - Alerts: {status.get('security_alerts_count', 0)}")
