...
class EnhancedVPNMeshDemo:
    ...
    async def _demonstrate_mesh_formation(self):
        ...
        for i in range(3):
            ...
            node_info = {
                ...
                "port": 51820 + i,
                "public_key": supervisor.node_public_key.hex() if supervisor.node_public_key else "",
                "role": "supervisor",
    ...
    async def _demonstrate_tunnel_management(self):
        ...
        for i, supervisor1 in enumerate(self.supervisor_nodes):
            for j, supervisor2 in enumerate(self.supervisor_nodes[i+1:], i+1):
                peer_info = {
                    ...
                    "port": 51820 + j,
                    "public_key": supervisor2.node_public_key.hex() if supervisor2.node_public_key else ""
                }
                
                success = await supervisor1.establish_tunnel_to_peer(supervisor2.node_id, peer_info)
                if success:
                    logger.info(f"   ✅ Tunnel established: {supervisor1.node_id} ↔ {supervisor2.node_id}")
                else:
                    logger.error(f"   ❌ Failed to establish tunnel: {supervisor1.node_id} ↔ {supervisor2.node_id}")
        ...
        for worker in self.worker_nodes:
            ...
            for supervisor in self.supervisor_nodes[:2]:
                ...
                if success:
                    logger.info(f"   ✅ Tunnel established: {worker.node_id} → {supervisor.node_id}")
        ...
        for node in all_nodes:
            ...
            logger.info(f"      Active tunnels: {status['active_tunnels']}/{status['total_tunnels']}")
            logger.info(f"      Protocol: {status['protocol']}")
            logger.info(f"      Success rate: {status['network_statistics']['tunnel_success_rate']:.1%}")
            ...
            if tunnel_details:
                for tunnel_id, details in tunnel_details.items():
                    if details['is_healthy']:
                        logger.info(f"   ✅ {tunnel_id}: Healthy (Latency: {details['latency_ms']:.1f}ms)")
                    else:
    ...
    async def _demonstrate_network_optimization(self):
        ...
        logger.info(f"      Success rate: {network_stats['healthy_tunnels']/max(1, network_stats['total_tunnels']):.1%}")
