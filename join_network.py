...
class DeviceDetector:
    ...
    def recommend_role(capabilities):
        ...
        if capabilities["is_server"] or (
            capabilities["has_gpu"] and capabilities["memory_gb"] >= 16
        ):
...
class NetworkJoiner:
    ...
    def display_capabilities(self, capabilities):
        ...
        device_type = (
            ...
            if capabilities["is_server"]
                ...
                else "Mobile" if capabilities["is_mobile"] else "Desktop"
            )
        )
    ...
    def get_user_preferences(self, recommended_role, recommended_stake, bootstrap_peer):
        ...
        for i, role in enumerate(
            [NodeRole.PEER, NodeRole.VERIFIER, NodeRole.MINER, NodeRole.SUPERVISOR]
        ):
    ...
    async def monitor_node(self):
        ...
        try:
            while True:
                ...
                print(
                    f"\r🟢 Running | Peers: {peer_count:2d} | Height: {blockchain_height:4d} | "
...
async def main():
    ...
    parser.add_argument(
        "--role", choices=["PEER", "VERIFIER", "MINER", "SUPERVISOR"], help="Node role"
    )
    parser.add_argument("--stake", type=float, help="Stake amount")
    parser.add_argument(
        "--bootstrap-peer", default="127.0.0.1:8000", help="Bootstrap peer address (host:port)"
    )
    parser.add_argument("--port", type=int, default=8000, help="Node port")
    parser.add_argument(
        "--auto", action="store_true", help="Use auto-detected settings without prompts"
    )
    ...
    if args.auto:
        ...
        node_id = (
            args.node_id
            or input(f"Node ID (default: my_{role.value.lower()}_001): ")
            or f"my_{role.value.lower()}_001"
        )
