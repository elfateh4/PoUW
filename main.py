...
logging.basicConfig(
    ...
    handlers=[
        ...
        (
            logging.FileHandler(app_config.monitoring.log_file)
            if app_config.monitoring.log_file
...
class PoUWApplication:
    ...
    def parse_node_role(self, role_str: str) -> NodeRole:
        ...
        if role_str not in role_mapping:
            raise ValueError(
                f"Invalid node role: {role_str}. Valid roles: {list(role_mapping.keys())}"
            )
    ...
    async def start_node(self, config: NodeConfig):
        ...
        try:
            ...
            self.running = True
            logger.info(
                f"PoUW node {config.node_id} started successfully on {config.host}:{config.port}"
            )
    ...
    def get_node_preferences(self, role: NodeRole) -> dict:
        ...
        if role == NodeRole.MINER:
            ...
            base_preferences.update(
                {"evaluation_capacity": 100, "specialized_metrics": ["accuracy", "f1_score"]}
            )
        ...
        try:
            while self.running:
                ...
                if self.node:
                    ...
                    logger.info(
                        f"Node status - Height: {status.get('blockchain_height', 0)}, "
    ...
    async def report_to_dashboard(self):
        ...
        try:
            ...
            node_data = {
                ...
                "stake": (
                    getattr(self.node.config, "initial_stake", 0)
...
async def main():
    ...
    parser.add_argument(
        "--bootstrap-peers", type=str, help="Comma-separated list of bootstrap peers (host:port)"
    )

    # Feature flags
    parser.add_argument("--enable-security", action="store_true", help="Enable security monitoring")
    parser.add_argument(
        "--enable-production", action="store_true", help="Enable production features"
    )
    parser.add_argument("--enable-advanced", action="store_true", help="Enable advanced features")
