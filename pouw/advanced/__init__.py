...
class ZeroNonceCommitment:
    ...
    def create_commitment(self, miner_id: str, future_iteration: int, 
        ...
        """
        commitment_id = hashlib.sha256(f"{miner_id}_{future_iteration}_{time.time()}".encode()).hexdigest()
    ...
    def verify_commitment_fulfillment(self, commitment_data: Dict[str, Any]) -> bool:
        ...
        expected_fulfillment_time = committed_at + (self.commitment_depth * 60)  # Assume 1 min per iteration
    ...
    def get_pending_commitments(self, miner_id: str) -> List[Dict[str, Any]]:
        ...
        return [
            commitment for commitment in self.pending_commitments.values()
...
if __name__ == "__main__":
    ...
    for worker in selected_workers:
        print(f"  - {worker['node_id']} (stake: {worker['stake_amount']}, GPU: {worker['has_gpu']})")
    
    # Verify selection
    selected_node_ids = [w['node_id'] for w in selected_workers]
    verification_result = worker_selector.verify_worker_selection("test_task_001", selected_node_ids, vrf_proofs)
