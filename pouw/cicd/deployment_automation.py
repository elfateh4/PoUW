...
class DeploymentPipelineManager:
    ...
    async def _deploy_to_kubernetes(
        self, config: DeploymentConfiguration, result: DeploymentResult
    ):
    ...
    def _generate_k8s_manifests(self, config: DeploymentConfiguration) -> Dict[str, Any]:
        ...
        if config.auto_scaling:
            hpa = {
                ...
                "metadata": {"name": f"{config.name}-hpa", "namespace": namespace},
                "spec": {
                    "scaleTargetRef": {
                        "apiVersion": "apps/v1",
                        "kind": "Deployment",
    ...
    async def _k8s_rolling_update(
        self, manifests: Dict[str, Any], config: DeploymentConfiguration, result: DeploymentResult
    ):
    ...
    async def _k8s_blue_green_deployment(
        self, manifests: Dict[str, Any], config: DeploymentConfiguration, result: DeploymentResult
    ):
        ...
        service_patch = {"spec": {"selector": {"app": config.name, "color": new_color}}}
    ...
    async def _k8s_canary_deployment(
        self, manifests: Dict[str, Any], config: DeploymentConfiguration, result: DeploymentResult
    ):
    ...
    async def _get_current_k8s_color(self, app_name: str, namespace: str) -> str:
        ...
        cmd = [
            ...
            f"app={app_name}",
            "-o",
            "jsonpath={.items[0].metadata.labels.color}",
        ]
        proc = await asyncio.create_subprocess_exec(
            *cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
        )
    ...
    async def _deploy_to_aws_lambda(
        ...
        try:
            ...
            self.lambda_client.update_function_code(
                FunctionName=function_name, ImageUri=config.image_tag
            )
...
class ReleaseManager:
    ...
    async def create_release(
        ...
        proc = await asyncio.create_subprocess_exec(
            "git", "rev-parse", "HEAD", cwd=self.project_root, stdout=asyncio.subprocess.PIPE
        )
        stdout, _ = await proc.communicate()
        ...
        await asyncio.create_subprocess_exec(
            "git", "tag", "-a", tag, "-m", f"Release {version}", cwd=self.project_root
        )
    ...
    async def _build_release_artifacts(self, version: str) -> List[str]:
        ...
        proc = await asyncio.create_subprocess_exec(
            "docker", "build", "-t", image_tag, ".", cwd=self.project_root
        )
        await proc.communicate()
    ...
    async def deploy_release(
        self, version: str, environment: Environment, deployment_manager: DeploymentPipelineManager
    ) -> DeploymentResult:
...
class PoUWDeploymentConfigurations:
    ...
    def staging() -> DeploymentConfiguration:
        ...
        return DeploymentConfiguration(
            ...
            replicas=2,
            auto_scaling={"min_replicas": 2, "max_replicas": 5, "cpu_target": 70},
        )
    ...
    def production() -> DeploymentConfiguration:
        ...
        return DeploymentConfiguration(
            ...
            replicas=5,
            auto_scaling={"min_replicas": 5, "max_replicas": 20, "cpu_target": 60},
...
async def main():
    ...
    release = await release_manager.create_release(
        version="1.0.0", release_notes="Initial production release"
    )
