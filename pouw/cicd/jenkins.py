...
class JenkinsfileGenerator:
    ...
    def create_ci_pipeline(self) -> PipelineConfiguration:
        ...
        stages = [
            PipelineStage(name="Checkout", steps=["checkout scm", "sh 'git clean -fdx'"]),
            PipelineStage(
            ...
            PipelineStage(
                ...
                parallel_stages={
                    ...
                    "Type Checking": PipelineStage(
                        name="Type Checking",
                        steps=["sh '. venv/bin/activate && mypy pouw --ignore-missing-imports'"],
                    ),
            ...
            PipelineStage(
                ...
                steps=["sh '. venv/bin/activate && python -m pytest tests/test_integration.py -v'"],
        ...
        return PipelineConfiguration(
            ...
            agent="any",
            stages=stages,
            triggers=["pollSCM('H/5 * * * *')", "cron('@daily')"],
    ...
    def create_docker_pipeline(self) -> PipelineConfiguration:
        ...
        stages = [
            PipelineStage(name="Checkout", steps=["checkout scm"]),
            PipelineStage(
            ...
            PipelineStage(
                ...
                post_actions={"always": ["sh 'docker system prune -f'"]},
            ),
        ]
    ...
    def create_deployment_pipeline(self) -> PipelineConfiguration:
        ...
        return PipelineConfiguration(
            ...
            environment={"KUBECONFIG": "/var/jenkins_home/.kube/config"},
        )
...
class JenkinsPipelineManager:
    ...
    def generate_jenkinsfile(
        self, pipeline: PipelineConfiguration, filename: str = "Jenkinsfile"
    ) -> Path:
...
"""
    ...
    def get_pipeline_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "validation_results": {jf.name: self.validate_jenkinsfile(jf) for jf in jenkinsfiles},
        }
