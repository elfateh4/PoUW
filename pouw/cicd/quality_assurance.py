...
class CodeQualityManager:
    ...
    async def analyze_quality(
        self, project_name: str = "PoUW", version: str = "latest"
    ) -> QualityReport:
                        ...
                        report.issues.append(
                            QualityIssue(
                                ...
                                severity=severity_map.get(
                                    issue.get("type", "info"), SeverityLevel.INFO
                                ),
    ...
    async def _analyze_documentation(self, report: QualityReport):
        ...
        report.metrics.documentation_coverage = (
    ...
    def _should_include_file(self, file_path: Path) -> bool:
        ...
        include_match = any(
            file_path.match(pattern) for pattern in self.configuration.include_patterns
        )
        ...
        exclude_match = any(
            file_path.match(pattern) for pattern in self.configuration.exclude_patterns
        )
        if exclude_match:
            return False
    ...
    def _get_metric_value(self, metrics: QualityMetrics, metric: QualityMetric) -> float:
        ...
        metric_map = {
            ...
            QualityMetric.SECURITY_VULNERABILITIES: len(
                [
                    i
                    for i in self.reports.get(
                        list(self.reports.keys())[-1],
                        QualityReport("", "", datetime.now(), QualityMetrics(datetime.now())),
                    ).issues
                    if i.check_type == QualityCheckType.SECURITY_SCAN
                    and i.severity == SeverityLevel.CRITICAL
                ]
            ),
        }
    ...
    def get_quality_trends(self, days: int = 30) -> Dict[str, List[Tuple[datetime, float]]]:
        ...
        trends = {"quality_score": [], "code_coverage": [], "complexity": [], "total_issues": []}

        for report in self.reports.values():
            if report.timestamp >= cutoff_date:
                trends["quality_score"].append(
                    (report.timestamp, report.summary.get("quality_score", 0))
                )
                trends["code_coverage"].append((report.timestamp, report.metrics.code_coverage))
                trends["complexity"].append(
                    (report.timestamp, report.metrics.cyclomatic_complexity)
                )
    ...
    async def generate_quality_report_html(
        self, report: QualityReport, output_path: str = "quality_report.html"
    ):
...
class SecurityScanner:
    ...
    async def _run_bandit_scan(self) -> List[QualityIssue]:
        ...
        try:
            cmd = ["bandit", "-r", "pouw/", "-f", "json", "-o", "/tmp/bandit_report.json"]
                ...
                for result in bandit_data.get("results", []):
                    ...
                    vulnerabilities.append(
                        QualityIssue(
                            ...
                            severity=severity_map.get(
                                result.get("issue_severity", "LOW"), SeverityLevel.LOW
                            ),
                            ...
                            confidence={"HIGH": 1.0, "MEDIUM": 0.7, "LOW": 0.3}.get(
                                result.get("issue_confidence", "LOW"), 0.5
                            ),
                        )
                    )
