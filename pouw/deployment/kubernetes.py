...
class KubernetesOrchestrator:
    ...
    async def deploy_pouw_components(self, components: Dict[str, ContainerConfiguration]) -> Dict[str, DeploymentStatus]:
    ...
    async def _deploy_component(self, component_name: str, config: ContainerConfiguration) -> DeploymentStatus:
    ...
    def _create_deployment_manifest(self, component_name: str, config: ContainerConfiguration) -> Dict[str, Any]:
    ...
    async def _wait_for_deployment(self, component_name: str, timeout: int = 300) -> bool:
        """Wait for deployment to be ready"""
        cmd = self.kubectl_cmd + ["rollout", "status", f"deployment/pouw-{component_name}", f"--timeout={timeout}s"]
    ...
    async def scale_deployment(self, component_name: str, replicas: int) -> bool:
        ...
        cmd = self.kubectl_cmd + ["scale", f"deployment/pouw-{component_name}", f"--replicas={replicas}"]
...
class PoUWDeploymentManager:
    ...
    def get_default_configurations(self) -> Dict[str, ContainerConfiguration]:
        ...
        return {
            ...
            'monitoring': ContainerConfiguration(
                ...
                ports=[3000, 9090],
                environment={
                    'POUW_METRICS_RETENTION': '7d',
                    'POUW_ALERT_ENABLED': 'true'
                },
                resources={
    ...
    def get_default_services(self) -> Dict[str, ServiceConfiguration]:
        ...
        return {
            ...
            'ml-trainer-api': ServiceConfiguration(
                ...
                ports=[
                    {'name': 'api', 'port': 8080, 'targetPort': 8080, 'protocol': 'TCP'}
                ],
    ...
    async def deploy_full_stack(self) -> Dict[str, Any]:
        ...
        try:
            ...
            all_components_ok = all(status == DeploymentStatus.RUNNING for status in component_status.values())
    ...
    async def scale_components(self, scaling_config: Dict[str, int]) -> Dict[str, bool]:
        ...
        for component, replicas in scaling_config.items():
            try:
                ...
                logger.info(f"Scaled {component} to {replicas} replicas: {'success' if success else 'failed'}")
    ...
    async def get_cluster_status(self) -> Dict[str, Any]:
        ...
        cluster_status = {
            ...
            'healthy_components': sum(1 for status in deployment_status.values() 
