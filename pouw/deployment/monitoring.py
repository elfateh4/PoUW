...
class MetricsCollector:
    ...
    async def _collect_system_metrics(self):
        ...
        self.metrics['system_memory_available'].append(
            Metric("system_memory_available", memory.available, MetricType.GAUGE, timestamp=timestamp)
        )
        
        # Disk metrics
        disk = psutil.disk_usage('/')
        self.metrics['system_disk_usage'].append(
            Metric("system_disk_usage", (disk.used / disk.total) * 100, MetricType.GAUGE, timestamp=timestamp)
        )
        
        # Network metrics
        network = psutil.net_io_counters()
        self.metrics['system_network_bytes_sent'].append(
            Metric("system_network_bytes_sent", network.bytes_sent, MetricType.COUNTER, timestamp=timestamp)
        )
        self.metrics['system_network_bytes_recv'].append(
            Metric("system_network_bytes_recv", network.bytes_recv, MetricType.COUNTER, timestamp=timestamp)
        )
    ...
    def get_metrics(self, metric_name: Optional[str] = None, 
                   time_range: Optional[timedelta] = None) -> Dict[str, List[Metric]]:
...
class LoggingManager:
    ...
    def get_structured_logs(self, time_range: Optional[timedelta] = None,
