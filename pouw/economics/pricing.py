...
class DynamicPricingEngine:
    ...
    def calculate_dynamic_price(self, 
        ...
        if market_metrics.total_demand > 0:
            ...
            if supply_demand_ratio < self.undersupply_threshold:
                ...
                supply_multiplier = 1.0 + (self.undersupply_threshold - supply_demand_ratio) * self.demand_elasticity
            elif supply_demand_ratio > self.oversupply_threshold:
                # Low demand, high supply - decrease price
                supply_multiplier = 1.0 - (supply_demand_ratio - self.oversupply_threshold) * self.supply_elasticity * 0.5
            else:
                # Balanced market
    ...
    def get_pricing_analytics(self) -> Dict[str, Any]:
        ...
        if len(recent_prices) >= 5:
            ...
            older_avg = statistics.mean(recent_prices[-10:-5]) if len(recent_prices) >= 10 else average_price
