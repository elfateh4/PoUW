    ...
    async def _create_network_interface(self, virtual_ip: str) -> bool:
        ...
        try:
            ...
            self.interfaces[interface_name] = NetworkInterface(
                interface_name=interface_name, virtual_ip=virtual_ip, is_active=True
            )
...
"""
    ...
    async def _establish_wireguard_tunnel(
        self, config: VPNTunnelConfig, peer_info: Dict[str, Any]
    ) -> bool:
...
"""
    ...
    async def _establish_openvpn_tunnel(
        self, config: VPNTunnelConfig, peer_info: Dict[str, Any]
    ) -> bool:
    ...
    async def _establish_generic_tunnel(
        self, config: VPNTunnelConfig, peer_info: Dict[str, Any]
    ) -> bool:
    ...
    def _start_network_monitoring(self) -> None:
        ...
        self.monitor_thread = threading.Thread(target=self._network_monitor_loop, daemon=True)
    ...
    def _is_node_reachable(self, node_id: str) -> bool:
        ...
        return (tunnel_id in self.connection_health and self.connection_health[tunnel_id]) or (
            reverse_tunnel_id in self.connection_health
            and self.connection_health[reverse_tunnel_id]
        )
    ...
    def get_mesh_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_tunnels": len(
                [t for t in self.tunnels.values() if t.state == TunnelState.CONNECTED]
            ),
            ...
            "network_statistics": {
                ...
                "average_latency": sum(config.latency_ms for config in self.tunnels.values())
                / max(1, len(self.tunnels)),
                "tunnel_success_rate": len([h for h in self.connection_health.values() if h])
                / max(1, len(self.connection_health)),
            },
        }
