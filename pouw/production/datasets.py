...
class ProductionDatasetManager:
    ...
    def load_torchvision_dataset(self, name: str, download: bool = True) -> DatasetMetadata:
        ...
        train_dataset = dataset_class(
            root=str(self.data_root), train=True, download=download, transform=transform
        )
        test_dataset = dataset_class(
            root=str(self.data_root), train=False, download=download, transform=transform
        )
        ...
        logger.info(
            f"Loaded {name} dataset: {metadata.size} samples, {metadata.num_classes} classes"
        )
        return metadata

    def load_csv_dataset(
        self, filepath: str, target_column: str, dataset_name: str = "custom_csv"
    ) -> DatasetMetadata:
        ...
        self.loaded_datasets[dataset_name] = {"full": dataset, "X": X, "y": y, "metadata": metadata}
        self.datasets[dataset_name] = metadata

        logger.info(
            f"Loaded CSV dataset {dataset_name}: {metadata.size} samples, {metadata.num_classes} classes"
        )
        return metadata
    ...
    def load_hdf5_dataset(
        ...
        logger.info(
            f"Loaded HDF5 dataset {dataset_name}: {metadata.size} samples, {metadata.num_classes} classes"
        )
        return metadata
    ...
    def split_dataset(
        ...
        if "full" in dataset_info:
            ...
            splits = {"train": train_set, "val": val_set, "test": test_dataset}
        ...
        logger.info(
            f"Split {dataset_name}: train={len(splits['train'])}, "
    ...
    def create_dataloaders(
        self, dataset_name: str, batch_size: int = 32, num_workers: int = 0
    ) -> Dict[str, DataLoader]:
...
def integrate_with_pouw_data_manager(
    dataset_manager: ProductionDatasetManager, pouw_data_manager, dataset_name: str
):
    """
    ...
    success = pouw_data_manager.store_dataset(dataset_id=dataset_name, data=serialized_data)
