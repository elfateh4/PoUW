...
class PerformanceMonitor:
    ...
    def setup_default_optimization_rules(self):
        ...
        def slow_execution_rule(metrics: List[PerformanceMetrics]) -> Optional[str]:
            ...
            if avg_time > 10.0:  # >10 seconds
                return (
                    "Slow execution detected. Consider GPU acceleration or algorithm optimization."
                )
            return None
        ...
        self.optimization_rules = [high_memory_usage_rule, slow_execution_rule, high_cpu_usage_rule]
    ...
    def record_metrics(self, metrics: PerformanceMetrics):
        ...
        if len(self.operation_stats[metrics.operation_name]) > self.history_size:
            self.operation_stats[metrics.operation_name] = self.operation_stats[
    ...
    def generate_performance_report(self) -> Dict[str, Any]:
        ...
        report = {
            "timestamp": time.time(),
            "system_health": {"current": current_health.__dict__, "trends": health_trend},
    ...
    def suggest_gpu_settings(self) -> Dict[str, Any]:
        ...
        if gpu_info:
            memory_usage_percent = (
                (gpu_info["memory_allocated"] / gpu_info["memory_reserved"]) * 100
                if gpu_info["memory_reserved"] > 0
                else 0
            )
            ...
            if memory_usage_percent > 80:
                suggestions["suggestions"].append(
                    "GPU memory usage high - consider reducing batch size"
                )
            elif memory_usage_percent < 30:
                suggestions["suggestions"].append(
                    "GPU memory underutilized - consider increasing batch size"
                )

            if "utilization" in gpu_info and gpu_info["utilization"] < 50:
                suggestions["suggestions"].append(
                    "GPU utilization low - consider model parallelization"
                )
