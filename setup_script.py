...
def run_command(
    command: List[str], description: str = None, capture_output: bool = False
    ...
    try:
        result = subprocess.run(
            command, capture_output=capture_output, text=True, check=True, cwd=project_root
        )
        if description:
...
def setup_python_environment():
    ...
    if not venv_path.exists():
        run_command([sys.executable, "-m", "venv", "venv"], "Creating Python virtual environment")
    ...
    if requirements_file.exists():
        run_command(
            [pip_cmd, "install", "-r", "requirements.txt"], "Installing Python dependencies"
        )
    else:
...
def validate_configuration(environment: str) -> bool:
    ...
    try:
        result = run_command(
            [sys.executable, "scripts/validate_config.py", "--environment", environment],
            f"Validating {environment} configuration",
            capture_output=True,
        )
...
def setup_docker_environment():
    ...
    directories = ["logs", "data", "cache", "nginx/ssl", "nginx/logs"]
    ...
    # Build Docker image
    try:
        run_command(["docker", "build", "-t", "pouw:latest", "."], "Building Docker image")
    except subprocess.CalledProcessError:
...
def run_tests():
    ...
    try:
        ...
        run_command([python_cmd, "-m", "pytest", "tests/", "-v"], "Running tests")
...
def main():
    ...
    parser.add_argument(
        "--skip-prereq-check", action="store_true", help="Skip prerequisite checking"
    )
    ...
    try:
        ...
        if not generate_environment_config(args.environment, interactive=not args.non_interactive):
            sys.exit(1)
