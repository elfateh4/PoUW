...
def sample_minibatch():
    ...
    return MiniBatch(batch_id="test_batch_001", data=data, labels=labels, epoch=0)
...
def distributed_trainer(simple_mlp):
    ...
    return DistributedTrainer(
        model=simple_mlp, task_id="test_task_001", miner_id="test_miner_001", tau=0.01
    )
...
def create_test_transaction(version=1, amount=10.0, address="test_user"):
    ...
    return Transaction(version=version, inputs=[], outputs=[{"address": address, "amount": amount}])
