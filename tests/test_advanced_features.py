...
from pouw.security import (
    ...
    SecurityAlert, AttackType
)
from pouw.crypto import (
    BLSThresholdCrypto, DistributedKeyGeneration, SupervisorConsensus,
    DKGState
)
from pouw.data import (
    ReedSolomonEncoder, ConsistentHashRing, DataAvailabilityManager,
    DataShard, DataShardType, DatasetSplitter
)
from pouw.advanced import (
    VerifiableRandomFunction, AdvancedWorkerSelection, ZeroNonceCommitment,
    MessageHistoryMerkleTree, VRFType, VRFProof
)
...
class TestByzantineFaultTolerance:
    ...
    def test_supervisor_voting_consensus(self):
        ...
        for i in range(4):
            consensus_reached = bft.submit_supervisor_vote(
                proposal_id, f"supervisor_{i}", True
            )
